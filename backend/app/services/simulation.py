...
def build_recommendations(
    ...
    Applies full diversity constraints (max 2 per slot, defensive injection,
    budget injection) and returns template-based explanations.  Prefer
    calling ``build_recommendations_async`` from async contexts when LLM
...
def _select_recommendations(
    ...
    Returns:
        List of up to 5 :class:`~app.models.recommendation.Recommendation`
        objects with template explanations, sorted by score descending.
    """
    if not simulations:
        return []
...
async def build_recommendations_async(
    simulations: list[SimulationResult],
    issues: list[CriticalIssue],
    build: BuildData,
    archetype: Archetype,
    ...
    """Select the top 5 recommendations, optionally enriching with LLM.
    ...
    * At least 1 cost-efficient option (< 1 divine) when available.
    ...
    When a ``session_id`` and ``llm_service`` are provided the function
    determines A/B group membership by hashing the session ID.  Sessions
    in the LLM group receive explanations generated by
    :class:`~app.services.llm_explainer.LLMExplainerService`; the rest
    receive the pre-rendered template string.
    ...
    Args:
        ...
        archetype: Detected build archetype.
        league: League name (for trade link generation).
        session_id: Frontend session UUID for A/B group assignment.
        llm_service: Optional LLM explainer service instance.

    Returns:
        List of up to 5 :class:`~app.models.recommendation.Recommendation`
        objects sorted by score (highest first).
    """
    # Run sync selection to produce template-based recommendations
    ...
    # ---- LLM explanation enrichment (M7) ------------------------------------
    use_llm = (
        llm_service is not None
        and session_id is not None
        and _is_llm_ab_group(session_id)
    )

    if use_llm and llm_service is not None:
        async def _enrich_with_llm(rec: Recommendation) -> Recommendation:
            """Replace template explanation with LLM-generated text.

            Args:
                rec: Recommendation with template-based explanation.

            Returns:
                Updated recommendation with LLM explanation (or original
                template if the LLM call fails or falls back).
            """
            text, source = await llm_service.generate_explanation(
                slot=rec.slot,
                current_item=rec.current_item,
                suggested_item=rec.suggested_item,
                category=rec.category,
                deltas=rec.deltas,
                price_divine=rec.price_divine,
                level=build.level,
                ascendancy=build.ascendancy or build.char_class,
                damage_type=archetype.damage_type,
                playstyle=archetype.playstyle,
                main_skill=build.main_skill or "",
                template_explanation=rec.explanation,
            )
            return rec.model_copy(
                update={"explanation": text, "explanation_source": source}
            )

        enriched = await asyncio.gather(
            *[_enrich_with_llm(rec) for rec in recommendations]
        )
...
def _is_llm_ab_group(session_id: str) -> bool:
    """Return True if the session_id falls in the LLM A/B group (50/50).

    Uses the first 4 hex digits of the MD5 hash of the session ID to
    produce a stable, uniformly-distributed assignment.

    Args:
        session_id: Frontend session UUID string.

    Returns:
        ``True`` if this session should receive LLM explanations.
    """
